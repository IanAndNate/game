  ...
  async ({ getState, setState }: StoreActionApi<State>) => {
    ...
    socket.on("keydown broadcast", (e: KeyPress) => {
      ...
      sampler.triggerAttack(
        e.note,
        Tone.now(),
        e.playerId === currentPlayerId ? 1 : 0.7
      );
      setState({
  ...
  ({ getState, setState, dispatch }: StoreActionApi<State>) => {
    ...
    socket.on("start round", ({ round, ...piece }: RoundInfo) => {
      ...
      const endTimer = setTimeout(() => {
        const { players: latestPlayers } = getState();
        setState({
...
export const keyDown =
  ...
  async ({ getState, setState }: StoreActionApi<State>) => {
    ...
    if (
      e.key === "Escape" &&
      [GameStatus.Starting, GameStatus.Running].includes(status)
    ) {
      if (escapeCount === 4) {
        // backdoor: hit escape consecutively 5 times to abort a round...
