...
const Song = ({ song, onChange }: SongProps) => {
  const { fileName, enabled, songNames, uniqueNotes, totalNotes, duration } =
    song;
  const [updatedSongNames, setUpdatedSongNames] = useState<string>(
    songNames.join(",")
  );
  useEffect(() => {
  ...
  const saveSongNames = (e: React.FormEvent) => {
    e.preventDefault();
    const doPatch = async () => {
      const cleaned = updatedSongNames
        .split(",")
  ...
  return (
    <SongRow>
      ...
      <td>
        <form onSubmit={saveSongNames}>
          <input
            type="text"
            value={updatedSongNames}
            onChange={(e) => {
          ...
          <input
            type="submit"
            value="save"
            disabled={updatedSongNames === songNames.join(",")}
          />
        </form>
      </td>
    </SongRow>
  ...
  return (
    <>
      ...
      <table>
        ...
        <tbody>
          {songs.map((s) => (
            <Song key={s.fileName} song={s} onChange={updateSong} />
          ))}
        </tbody>
      </table>
      ...
      <div>
        <form onSubmit={fetchUrl}>
          ...
          />
          <button type="submit" disabled={url.length < 8}>
            add MIDI from URL
          </button>
        </form>
      </div>
