...
const moveVertically = (duration: number) => keyframes`
    ...
    100% {
        transform: scale(1) rotateX(45deg) translate3d(0, ${
          duration * 500
        }px, 0);
    }
`;
...
const MusicPage = styled.div<{
  ...
  animation: ${({ duration }) => moveVertically(duration)}
    ${({ duration, speedFactor }) => duration * speedFactor}s linear;
...
export const Play = () => {
  ...
  const { song, notes, speedFactor, totalDuration } = piece || {
    song: null,
    piece: null,
  };
  ...
  return (
    ...
    >
      ...
      <MusicContainer ref={renderTarget} height={targetHeight}>
        ...
        >
          {song &&
            song
              .filter(({ key }) => !!key)
              .map(({ key, time, duration: noteDuration }, i) => (
                <Note
                  key={i}
                  index={i}
                  time={time}
