...
const TimedBar = styled.div<{ duration: number }>`
  ...
  height: 4px;
  animation: ${progressBar} ${({ duration }) => Math.ceil(duration / 1000)}s
    linear;
`;
...
const TimeLeft = ({ timeTillLaunch, totalDuration, speedFactor }: Props) => {
  ...
  }, [timeTillLaunch, totalSecs]);
  return (
    <TimeLeftContainer>
      {Math.floor(secondsToEnd / 60)}:
      {String(secondsToEnd % 60).padStart(2, "0")}
    </TimeLeftContainer>
  );
};
...
export const ProgressIndicator = (props: Props) => {
  ...
  return (
    <Container>
      <TimeLeft {...props} />
      {timeTillLaunch === 0 && (
        <TimedBar duration={totalDuration * speedFactor} />
      )}
    </Container>
  );
};
